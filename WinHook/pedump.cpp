...
int parse_import_directory(const pedump_t* pe, const uint8_t* buf, uint64_t length, int verbose, int no_print)
{
    ...
    for (;;imp++) {
        ...
        if (pe->FileHeader.Machine == IMAGE_FILE_MACHINE_I386) {
            for (thunk32 = (PIMAGE_THUNK_DATA32)(buf + offset);thunk32->u1.Ordinal;thunk32++) {
                ...
                imports.push_back(std::string(function->Name));
            }
    ...
    for (i = 0;i < exp->NumberOfFunctions;i++) {
        ...
        if (j == exp->NumberOfNames) {
            printf("    0x%04x   0x%04x   0x%08x (NONAME)\n",
                    (unsigned int)(exp->Base + i), (unsigned int)j, (unsigned int)(rva2offset(pe, func_table[i])));
            continue;
        }
        ...
        printf("    0x%04x   0x%04x   0x%08x %s\n",
                (unsigned int)(exp->Base + i), (unsigned int)j, (unsigned int)(rva2offset(pe, func_table[i])), (char*)(buf + offset));
    }
...
int parse_pe(const uint8_t* buf, uint64_t length,
        int show_imports, int show_exports, int verbose, int no_print)
{
    pedump_t pe;
    int i;
    uint32_t offset;
    ...
    if (verbose && show_imports && show_exports && no_print==0) {
        printf("  Sections\n");
        printf("    %-10s %-10s\n", "RVA", "OFFSET");
        printf("    %-10s %-10s\n", "----------", "----------");
    ...
    if (show_imports) {
        parse_import_directory(&pe, buf, length, verbose, no_print);
    }
    if (show_exports) {
...
int pedump(const char* fn, int show_imports, int show_exports, int verbose, int no_print)
{
    HANDLE hfile;
    HANDLE hmap;
    ...
    printf("%s: ", fn);
    parse_pe(buf, length, show_imports, show_exports, verbose, no_print);
...
int _pe_main(int argc, char* argv[]) {
    ...
    for (i = 0;i < cfg->opts_n;i++) {
        if (strcmp(cfg->opts[i].key, "-v") == 0) {
            verbose = 1;
			no_print = 0;
        } else if (strcmp(cfg->opts[i].key, "-vnp") == 0) {
            verbose = 1;
            no_print = 1;
            no_exports = 1;
    ...
    for (i = 0;i < cfg->args_n;i++) {
        pedump(cfg->args[i], show_imports, show_exports, verbose, no_print);
    }
...
int dumpExe(char* filename) {
  char *argv[] = {(char*)"pedump.exe", (char*)"-v", filename};
  return _pe_main(3, argv);
}

bool ifApiExists(char* filename, std::string apiname) {

	char *argv[] = {(char*)"pedump.exe", (char*)"-vnp", filename};
	_pe_main(3, argv);

	for(int i=0; i<(int)imports.size(); i++) {
		
		std::string temp = imports.at(i);

		//cout << temp << apiname << endl;

		if (apiname.compare(temp) == 0) {
			return true;
		}
	}
